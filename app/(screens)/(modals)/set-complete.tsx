    ...
    <View style={[styles.modalOverlay, { backgroundColor: 'rgba(0, 0, 0, 0.5)' }]}>
      ...
      >
        <View style={styles.modalHeader}>
          <Text size="xl" weight="semibold">Set Complete!</Text>
          <TouchableOpacity 
            onPress={handleClose}
        ...
        <View style={styles.setStats}>
          ...
          <View style={[styles.statsCard, { backgroundColor: colors.background.card.primary }]}>
            ...
            <View style={styles.statItem}>
              <Text weight="semibold">Set Average</Text>
              <Text variant="secondary">{winner.gameAvg.toFixed(1)}</Text>
            </View>
            <View style={styles.statItem}>
              <Text weight="semibold">Legs Won</Text>
              <Text variant="secondary">{winner.legs}</Text>
            </View>
            {players.length > 1 && (
              <View style={styles.statItem}>
                <Text weight="semibold">Legs Lead</Text>
                <Text variant="secondary">
                  +{winner.legs - Math.max(...players.filter(p => p.id !== winner.id).map(p => p.legs))}
                </Text>
              </View>
            )}
          </View>
